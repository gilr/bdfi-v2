...
<div class='grid grid-cols-1 mx-2 sm:ml-5 sm:mr-2 md:ml-10 md:mr-4 px-2 sm:pl-5 sm:pr-2 md:pl-10 md:pr-4'>
    ...
    <div class='text-base'>
        Editeur  : <span class='font-semibold'><a class='border-b border-dotted border-purple-700 hover:text-purple-700 focus:text-purple-900' href='/editeurs/{{ $publisher->id }}'>{{ $publisher->name }}</a></span>
    </div>
        ...
        <div class='text-base'>
            @foreach ($results as $publication)
                <div class='ml-2 md:ml-8'>
                    <a class='border-b border-dotted border-purple-700 hover:text-purple-700 focus:text-purple-900' href='/ouvrages/{{ $publication->id }}'>{{ $publication->name }}</a>,
                    ...
                    de
                        @foreach($publication->authors as $author)
                            @if (!$loop->first)
                                ,
                            @endif
                            <a class='text-red-800 border-b border-dotted border-purple-700 hover:text-purple-700 focus:text-purple-900' href='/auteurs/{{ $author->id }}'>{{ $author->fullname }}</a> <span class='hidden xl:inline'>({{ $author->pivot->role->getLabel() }})</span>
                        @endforeach
                    @endif
