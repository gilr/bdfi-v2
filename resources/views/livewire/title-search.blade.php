    ...
    @if($showModal)
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center shadow-lg shadow-indigo-500/50">
            <div class="bg-white p-6 rounded-lg shadow-lg w-9/12">
                <h2 class="text-lg font-semibold mb-4">Rechercher un titre</h2>
                ...
                @if($titles && $titles->isNotEmpty())
                    <!-- Liens de pagination -->
                    <div class="mt-4">
                        Cliquer sur un titre ouvre sa page dans un nouvel onglet.
                    </div>
                    <div class="mt-4">
                        {{ $titles->links() }}
                    </div>
                    ...
                    <div class='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-0 lg:gap-0.5 border py-2"'>
                        @foreach($titles as $title)
                            <div>
                                <x-admin.link-ext lien='/textes/{{ $title->slug }}'>
                                    {{ $title->name }}, {{ $title->copyright }} {{ $title->title_vo }}
                                </x-admin.link-ext>
                            </div>
                        @endforeach
                    </div>
