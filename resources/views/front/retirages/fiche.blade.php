<div class='grid grid-cols-1 lg:grid-cols-2 gap-0.5 bg-gradient-to-b from-yellow-400 via-pink-500 to-purple-500 mx-2 sm:ml-5 sm:mr-2 md:ml-10 md:mr-4'>
    ...
    <div class='bg-gray-100 px-2 sm:pl-5 sm:pr-2 md:pl-10 md:pr-4'>
        {{-- zone couverture --}}
        @php
            $image = array('url' => "https://www.bdfi.info/couvs/" . InitialeCouv($results->publication->cover_front) . "/" . $results->publication->cover_front . ".jpg", 'name' => $results->publication->name);
        @endphp
        <livewire:cover-zoom :image="$image" />
    </div>
</div>
